;+
; NAME:
;     Emission_Angle_Correction_1_74
;
; PURPOSE:
;     Correction of Emission Angle for VIRTIS calibrated data at 1.74 microns
;
...
;
;     Correction coefficients corrected based on new publication
;        "Limb Darkening study using Venus nightside infrared spectra from VIRTIS-Venus
;         Express data"
;         Andrea Longobardo, Ernesto Palomba, Angelo Zinzi, Giuseppe Piccioni, 
;         Constantine Tsang, Pierre Drossart
;
; CALLING SEQUENCE:
;     emission_angle_correction_1_74, QUBE=input_cube, EMband=input_EMband, CORRECTED_QUBE=cube, /NO_POPUPS
;
; INPUTS:
...
;         > emission_angle_correction_1_74, QUBE=band, EMband=EM, CORRECTED_QUBE=corrected_cube
;
; REQUIRED FILES:
...
PRO emission_angle_correction_1_74, event, QUBE=input_cube, EMband=input_EMband, CORRECTED_QUBE=cube, NO_POPUPS=no_popups
...
for b=0,nb-1 do begin
    ...
    cube[*,*,b] = cube[*,*,b] / (0.34  + 0.66 *cos(EMband)) ; backscatter correction (LONGOBARDO)
;	cube[*,*,b] = cube[*,*,b] / (0.28  + 0.72 *cos(EMband)) ; backscatter correction (LONGOBARDO) WRONG VALUES
;	cube[*,*,b] = cube[*,*,b] / (0.316 + 0.685*cos(EMband)) ; backscatter correction (CARLSON)
endfor
;---------------------------------------------------

...
PRO accumulated_projection, DEBUG=debug, NO_DISPLAY=no_display
...
;;==================================================================
 ...
 ;path_in    = "ARCHIVE/DATA/MTP008/"
 path_in    = "C:\Documents and Settings\acardesin\My Documents\VIRTIS\PDS_Test\VIR0212\" ; path of input data (e.g. "V:\DATA\MTP004\VIR0108"; "V:\DATA")
 search_key = "VI0"    ; filter key (e.g. "VI" for all infrared, "VI0345" full orbit 345, "VI034[4-7]" orbits 344-347 , "VI0345_00" one single cube, "VI0345_0[0-7]" cubes 00 to 07 from this orbit)

 ; Path to save the IDL variables and ENVI images (if needed)
 path_save  = "C:\Documents and Settings\acardesi\Desktop\test\"
 ;path_save  = "~/Accumulated_projection_analysis/"
 ; Path to save the PNG image of each grid (if needed)
 path_png   = "C:\Documents and Settings\Utente\Desktop\Alex_temp\Accumulated_projection_analysis\PNGs\"

;;==================================================================
;; Define Suffix of output files
;;==================================================================

 suffix              = 'TB_3_8um_LOCALTIME_maxEM80_trigrid' ;suffix for the PNG files, IDL save files and ENVI image files
...
;;==================================================================
 Thermal_Brightness  =  1 ; set to transform Radiance into Thermal Brightness (Note: continuum subtraction is not performed)
 ...
 average             =  0 ; 1 to average all selected bands (default), 0 to integrate bands (average is performed when Thermal Brightness is set)
 ...
 max_emergence_angle = 80.; max emergence angle to be considered, 90 for all, ~80 to avoid limbs, 40~60 to avoid emergence angle variations
 min_incidence_angle =  0.; min incidence angle to be considered,  0 for all, ~95 for only nightside
 ...
 min_distance        =  0.; min slant distance (in km), 0   for all, 30000-50000 for only low-resolution  (apocenter) observations
 max_distance        = 1e6; max slant distance (in km), 1e6 for all, 10000-20000 for only high-resolution (pericenter) observations
...
;;==================================================================
 Color_Table     = 5 ; color table for the plot/contour/map
 disp_ENVI       = 1 ; (recommended) use ENVI to display averaged grids and results (ENVI files are created in any case)
 disp_MAP        = 1 ; (recommended) use MAP projection to display averaged grid (it can be done later from ENVI)
 ...
 ;range=[0   , 0.6] ;typical for nightside 1.7um
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ...
 IF countqubes EQ 0 THEN message, "ERROR: no CAL qubes found matching key"

 filter    = "*"+search_key+"*.GEO"
 geo_list  = FILE_SEARCH(path_in, filter, count=countgeos)
 IF countgeos  EQ 0 THEN message, "ERROR: no GEO qubes found matching key"
...
;======================================================================
 FOR i=0,countqubes-1 DO BEGIN
	...
	IF (FILE_BASENAME(qube_file) EQ 'VI0042_00.CAL' ) || $
	   (FILE_BASENAME(qube_file) EQ 'VI0040_00.CAL' ) || $
	   (FILE_BASENAME(qube_file) EQ 'VI0520_04.CAL' ) || $
	   (FILE_BASENAME(qube_file) EQ 'VI0520_06.CAL' ) || $
	   (FILE_BASENAME(qube_file) EQ 'VI0385_07.CAL' ) || $
	   (FILE_BASENAME(qube_file) EQ 'VI0444_08.CAL' ) || $
	   (FILE_BASENAME(qube_file) EQ 'VI0505_05.CAL' ) || $
	   (FILE_BASENAME(qube_file) EQ 'VI0503_07.CAL' ) || $
	   (FILE_BASENAME(qube_file) EQ 'VI0344_03.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0339_06.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0333_00.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0342_04.CAL' ) || $   ; Occultations
	   (FILE_BASENAME(qube_file) EQ 'VI0390_10.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0390_09.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0332_03.CAL' ) || $  _;
	   (FILE_BASENAME(qube_file) EQ 'VI0341_06.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0340_06.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0331_03.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0342_06.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0346_05.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0343_06.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0300_04.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0380_02.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0325_03.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0579_04.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0302_00.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0479_03.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0112_01.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0102_15.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0366_03.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0307_02.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0335_00.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0345_06.CAL' ) || $   ; Files with spikes
	   (FILE_BASENAME(qube_file) EQ 'VI0347_06.CAL' ) || $   ;  or wrong values
	   (FILE_BASENAME(qube_file) EQ 'VI0349_06.CAL' ) || $   ; that make my map ugly
	   (FILE_BASENAME(qube_file) EQ 'VI0348_04.CAL' ) || $   ;(they should be double checked)
	   (FILE_BASENAME(qube_file) EQ 'VI0381_05.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0380_06.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0476_03.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0479_05.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0298_03.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0332_06.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0334_03.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0335_06.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0337_06.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0337_04.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0336_07.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0137_15.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0467_02.CAL' ) || $   ;
  	   (FILE_BASENAME(qube_file) EQ 'VI0337_03.CAL' ) || $   ;
	   (FILE_BASENAME(qube_file) EQ 'VI0374_05.CAL' ) || $  _;
	   ...
	   (FILE_BASENAME(qube_file) EQ 'VI0155_01.CAL' ) THEN CONTINUE
...
;	accumulated_grid = accumulated_grid[*,*,1:*]
 ...
 ENDIF ELSE message, "Error: no file was processed"

...
PRO phase_angle_correction, event, QUBE=input_cube, IAband=input_IAband, CORRECTED_QUBE=cube, NO_POPUPS=no_popups
...
IF N_ELEMENTS(input_IAband) EQ 0 THEN BEGIN
	...
	PAband = select_cube_from_envi(/BAND, TITLE="Select band containing Phase angle values", CANCEL=cancel)
	if cancel then return

ENDIF ELSE IABand= input_IABand

PAband = !pi/180.*float(PAband) ;in RADIANS
;---------------------------------------------------
...
for b=0,nb-1 do begin
	...
	cube[*,*,b] = cube[*,*,b] / sin(PAband); / cos(EAband)*0.25 ; IA and EA cosine law correction

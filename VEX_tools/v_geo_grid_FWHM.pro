...
FUNCTION v_geo_grid_fwhm, qube_file, geo_file                     ,$
...
ENDIF ELSE BEGIN
    ...
    If Keyword_Set(Rayleigh) Then Begin
       rad_to_rayleigh, QUBE=cubefiltered, WL=(qube.table)[*,0,0], Rayleigh=rayleigh_cube, NO_POPUPS=no_popups
       cubefiltered = rayleigh_cube
    endif

    ; Thermal Brightness: convert to thermal brightness
    If Keyword_Set(Thermal_Brightness) Then Begin
       compute_thermal_brightness, QUBE=cubefiltered, WL=(qube.table)[*,0,0], TB=TB, NO_POPUPS=no_popups
       cubefiltered = TB
    endif
    ...
    ; RADIANCE (get Radiance band from qube) -----------------
    ...
    ; FWHM (get only FWHM instead of radiance values) ---------

    IF KEYWORD_SET(FWHM) THEN BEGIN
        ; find maximum and minimum
        maximum=max(bands,/NaN, DIM=1, MIN=minimum)

        ; find half maximum (taking continuum into account)
        if N_ELEMENTS(AVGContinuum) eq 1 then $
            half_max = (maximum-AVGContinuum)/2.+AVGContinuum $
        else $
            half_max = (maximum-minimum)/2.+minimum

        higher = bands

        ; find bands higher than half maximum
        for b=0,N_ELEMENTS(iband)-1 do higher[b,*,*] = (bands[b,*,*] gt half_max[b])

        ; number of bands over half maximum
        band_fwhm = total(higher,1,/NaN)
...
; Check Saturated and Negative Pixels -------------------------------------------
IF only_positive THEN minimum = 0 ELSE minimum = -999
ValidDataIndex=where(Radiance[ValidGeometryIndex] GE minimum, ValidDataCount)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ...
 index_band    = [72,73,74,75,76,77,78,79,80] ; index of band or bands to use
...
;; Define file paths
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
qube_file="E:\ARCHIVE\DATA\MTP001\VIR0025\CALIBRATED\VI0025_07.CAL"
geo_file ="E:\ARCHIVE\DATA\MTP001\VIR0025\GEOMETRY\VI0025_07.GEO"
;qube_file="E:\ARCHIVE\DATA\MTP021\VIR0577\CALIBRATED\VI0577_10.CAL"
;geo_file ="E:\ARCHIVE\DATA\MTP021\VIR0577\GEOMETRY\VI0577_10.GEO"
...
;; GRID DATA
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
grid=v_geo_grid_fwhm(qube_file, geo_file, INDEX_BAND=index_band, $
                     INDEX_CONTINUUM    = index_continuum    ,$
                     INDEX_THERMAL      = index_thermal      ,$
                     INDEX_CONT_THERMAL = index_cont_thermal ,$
                     THERMAL_RATIO      = thermal_ratio      ,$

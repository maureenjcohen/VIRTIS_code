...
FUNCTION v_geo_grid, qube_file, geo_file                     ,$
  ...
  ENDIF ELSE BEGIN
    ...
    ;#### INTERPOLATION FOR CO BANDS (2.29-2.32 Tsang 2007)
    IF KEYWORD_SET(interpolate_co) THEN BEGIN
      dims = SIZE(cubefiltered, /DIMENSIONS)
      FOR line = 0, dims[2]-1  DO BEGIN
        ;print, qube.table[132:137,line,0]
        IF interpolate_co EQ 1 THEN $
          cubefiltered[132:137,*,line] = interpIntegrate_3D((qube.table)[132:137,line,0], cubefiltered[132:137,*,line], 0, 5, $
          [qube.table[132,line,0], 2.2900, 2.3000, 2.3100, 2.3200, qube.table[137,line,0]],/DO_CHECK)
        ;print, qube.table[130:140,line,0]
        IF interpolate_co EQ 2 THEN $
          cubefiltered[130:139,*,line] = interpIntegrate_3D((qube.table)[130:139,line,0], cubefiltered[130:139,*,line], 0,9, $
          [qube.table[130,line,0], qube.table[131,line,0], qube.table[132,line,0], 2.2900, 2.3000, 2.3100, 2.3200, $
          qube.table[137,line,0], qube.table[138,line,0], qube.table[139,line,0]])
      ENDFOR
    ENDIF

    ;#### INTERPOLATION FOR H2O BANDS (2.53-2.56 Barstow 2012)
    IF KEYWORD_SET(interpolate_h2o) THEN BEGIN
      dims = SIZE(cubefiltered, /DIMENSIONS)
      FOR line = 0, dims[2]-1  DO BEGIN
        ; print, qube.table[159:164,line,0]
        IF interpolate_h2o EQ 1 THEN $
          cubefiltered[159:164,*,line] = interpIntegrate_3D((qube.table)[159:164,line,0], cubefiltered[159:164,*,line], 0, 5, $
          [qube.table[159,line,0], 2.5300, 2.5400, 2.5500, 2.5600, qube.table[164,line,0]],/DO_CHECK)
        ; print, qube.table[157:166,line,0]
        IF interpolate_h2o EQ 2 THEN $
          cubefiltered[157:166,*,line] = interpIntegrate_3D((qube.table)[157:166,line,0], cubefiltered[157:166,*,line], 0,9, $
          [qube.table[157,line,0], qube.table[158,line,0], qube.table[159,line,0], 2.5300, 2.5400, 2.5500, 2.5600, $
          qube.table[164,line,0], qube.table[165,line,0], qube.table[166,line,0]])
      ENDFOR
    ENDIF
    ...
    ;########### CAREFUL introduces NAN values, turned to 0 by TOTAL below ## BUG ??? ##
    ...
    If Keyword_Set(Median_Filter) Then begin
      ...
      for b=0,N_ELEMENTS(iratio)-1 do RATIOBANDS[b,*,*]=MEDIAN(reform(RATIOBANDS[b,*,*]),3)
    endif

    ; INTEGRATE Radiance Bands
    Radiance = total(BANDS,1)

    ; using /NaN: total of NaN is zero and NaN values dissappear## BUG ??? ##
    ;Radiance = total(BANDS,1,/NAN)
    ;restore NaN values
    ;izeros = where(Radiance EQ 0, countZeros)
    ;if countzeros GT 0 then Radiance[izeros]=!values.F_Nan
    ...
    if N_ELEMENTS(ratiobands) GT 0 then begin

      Ratio = total(ratiobands,1)
      ;using /NaN: total of NaN is zero and NaN values dissappear## BUG ??? ##
      ;Ratio = total(ratiobands,1,/NAN)

      If Average || Keyword_Set(Thermal_Brightness) then begin
        counter_ratio  = float(total(FINITE(ratiobands),1,/NAN))
        Ratio   = Ratio/counter_ratio ;Total ratiobands divided by counter is the mean value
      endif
